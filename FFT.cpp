...
void FFT::setBufferedSignalValues(std::vector<std::deque<float>> vBufferedSignalValues)
{
    ...
    for (auto l_signal = 0; l_signal < m_i32NbSignals; l_signal++)
    {
        ...
        qDebug() << "Peak at : " <<  findPowerSpectrumPeak(l_vFFTPowerSpectrum) << " Hz";
        // Phase Spectrum
...
float FFT::findPowerSpectrumPeak(std::vector<float> &vFFTPowerSpectrum)
{
    std::vector<float>::iterator result;
    result = std::max_element(vFFTPowerSpectrum.begin(), vFFTPowerSpectrum.end());
