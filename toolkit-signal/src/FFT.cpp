...
FFT::FFT()
{
    ...
    m_fMinFrequencyRange = 0.0;
    m_fMaxFrequencyRange = 0.0;
    m_fMinPeakAmplitudeThreshold = 0.0;
}
...
void FFT::setBufferedSignalValues(std::vector<std::deque<float>> vBufferedSignalValues)
{
    ...
    for (auto l_signal = 0; l_signal < m_i32NbSignals; l_signal++)
    {
        ...
        //qDebug() << "Peak at : " <<  findPowerSpectrumPeak(l_vFFTPowerSpectrum, m_fMinFrequencyRange, m_fMaxFrequencyRange) << " Hz";
        // Phase Spectrum
...
void FFT::setFrequencyRange(float fMinFrequencyRange, float fMaxFrequencyRange)
{
    m_fMinFrequencyRange = fMinFrequencyRange;
    m_fMaxFrequencyRange = fMaxFrequencyRange;
}

void FFT::setMinPeakAmplitudeThreshold(float fMinPeakAmplitudeThreshold)
{
    m_fMinPeakAmplitudeThreshold = fMinPeakAmplitudeThreshold;
}
...
float FFT::findPowerSpectrumPeak(std::vector<float> &vFFTPowerSpectrum, float lowFrequencyRange, float highFrequencyRange, float minPeakAmplitudeThreshold )
{
    // determines the start index
    int startIndex = lowFrequencyRange * vFFTPowerSpectrum.size() / m_fFps;
    std::vector<float>::iterator itStart = vFFTPowerSpectrum.begin();
    std::advance (itStart, startIndex);

    // determines the end index
    int endIndex = highFrequencyRange * vFFTPowerSpectrum.size() / m_fFps;
    std::vector<float>::iterator itEnd = vFFTPowerSpectrum.begin();
    std::advance (itEnd, endIndex);

    // finds the max value between the start and end indices
    std::vector<float>::iterator result;
    result = std::max_element(itStart, itEnd);

    // checks if the max value is a peak by comparing its value to a threshold
    if (*result < minPeakAmplitudeThreshold)
        return 0.0;

    // converts the index to frequency before returning
    return (float)std::distance(vFFTPowerSpectrum.begin(), result) / (float)vFFTPowerSpectrum.size() * m_fFps;
}
